...
export default function Certificates() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await certificates.update(editing.id, form)
        toast.success('Certificate updated.')
      } else {
        await certificates.add(form)
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.error(err.message || 'Could not remove certificate.')
    }
  }
    ...
    <div className="mx-auto max-w-5xl">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((c) => (
            <CertificateCard key={c.id} cert={c} onEdit={openEdit} onDelete={handleDelete} />
          ))}
        </div>
      )}
